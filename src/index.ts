...
type MethodAndAlias = `${_HTTPMethods}${string}`;
...
type APIPath<Config extends APIConfig, Name = keyof Config> = U.ListOf<
    ...
    ? {
        // ! 这里有错误, Method 被错误的推断成了一个字母 g
        // alias: `${Lowercase<Method>}${Capitalize<Alias>}`;
        alias: `${Lowercase<Method>}${Alias}`;
...
export const api = <Config extends APIConfig>(config: Narrow<Config>) => {
    ...
    for (const [key, _value] of Object.entries(config)) {
        ...
        const pattern = key.match(/^(get|post|put|delete)/);
        const method = pattern ? pattern[0] : null;
        const alias = method ? key.slice(method.length) : null;
